...
class GUIPattern():
    ...
    def save(self):
        ...
        with open(Path(folder) / 'design_params.json', 'w') as f:
            dict = read_yaml_to_dict(Path(folder) / 'design_params.yaml')
            str = json.dumps(dict,sort_keys=False,indent=4,separators=(',',': '))
            f.write(str)
            f.close()
